...
ArduinoUniqueID::ArduinoUniqueID()
{
  ...
  // Read device ID from FICR (Factory Information Configuration Registers)
  // The nRF52840 has two 32-bit device ID registers
  uint32_t deviceid_low = NRF_FICR->DEVICEID[0];
  uint32_t deviceid_high = NRF_FICR->DEVICEID[1];
  
  // Store as 8 bytes (64-bit unique ID)
  id[0] = (deviceid_high >> 24) & 0xFF;
  id[1] = (deviceid_high >> 16) & 0xFF;
  id[2] = (deviceid_high >> 8) & 0xFF;
  id[3] = deviceid_high & 0xFF;
  id[4] = (deviceid_low >> 24) & 0xFF;
  id[5] = (deviceid_low >> 16) & 0xFF;
  id[6] = (deviceid_low >> 8) & 0xFF;
  id[7] = deviceid_low & 0xFF;
